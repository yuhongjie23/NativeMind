...
export function createApplication(deps: ApplicationDeps): Application {
  ...
  const useCases: Application['useCases'] = {
    ...
    deleteNote: new DeleteNoteUseCase(repos.note, infra.vectorCleanup, confirmation, eventBus),
  ...
  const unsubscribes = [
    ...
    // 不再自动生成复盘并静默落库：AI 写的复盘必须经确认门（AGENTS.md 铁律）。
    // 复盘由用户主动生成（ReviewPage / 全屏复盘面板），或午夜自动生成（App.tsx，用户已确认的既定行为）。
