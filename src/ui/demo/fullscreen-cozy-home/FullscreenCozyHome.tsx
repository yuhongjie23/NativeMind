...
const runMaintenance = async (): Promise<void> => {
  ...
  try {
    ...
    if (last === today) return; // 今天已跑过，不重复
    ...
    await Promise.allSettled([
      ...
      // 每日备份一份数据库快照（VACUUM INTO 一致快照，覆盖保留最近一份）
      import('@tauri-apps/api/core').then(({ invoke }) => invoke('db_backup')),
    ]);
...
export function FullscreenCozyHome() {
  ...
  const toggleBgm = () => {
    ...
    // 音乐库有选中曲目 → 播放/暂停它
    if (library.current) {
      void (library.playing ? library.toggle() : library.play(library.current));
      return;
    }
