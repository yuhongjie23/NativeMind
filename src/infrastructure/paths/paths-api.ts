...
/**
 * 检查一个目录是否可用（存在 + 是目录 + 可写）。
 * 兼容中文路径；失败返回结构化结果而不是抛错，前端直接展示原因。
 */
export const checkPath = async (path: string): Promise<PathCheckResult> => {
  if (!isTauri() || !path.trim()) {
    return { ok: false, exists: false, isDir: false, reason: '未配置路径' };
  }
  return invoke<PathCheckResult>('file_check_path', { path: path.trim() });
};
