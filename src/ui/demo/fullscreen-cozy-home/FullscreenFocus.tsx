...
export function FullscreenFocus({ onClose, volume, muted, onVolumeChange, onToggleMute }: FullscreenFocusProps) {
  ...
  const exitFocus = async () => {
    if (activeRef.current) {
      const ok = await useConfirmationStore.getState().requestSimple({
        title: t('退出专注'),
        // 实际行为：退出全屏后会话在后台继续计时，到点照常完成并统计。
        // 文案如实说明，避免「关掉就停了」的误解；真想停用「放弃」。
        message: t('退出全屏后这段专注会在后台继续，到点自动完成；想现在停掉请用「放弃」。'),
        confirmLabel: t('退出'),
        danger: true,
      });
      if (ok) onClose();
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      // 空格：空闲开始专注；专注中暂停/恢复。
      // 焦点在按钮/输入框等交互元素上时不劫持——否则 Tab 到「完成」按钮按空格
      // 会触发暂停而不是点击按钮。
      if (event.code === 'Space') {
  ...
  const onStart = async () => {
    await start({ durationMinutes: duration, todoId: selectedTodoId });
  };
    ...
    <div className="focus-overlay" role="dialog" aria-modal="true" aria-label="专注模式">
      ...
      <div className="focus-overlay__center">
        ...
        <p className="focus-overlay__task">
          {active
            ? active.todoId
              ? t('专注中：{0}', todos.find((todo) => todo.id === active.todoId)?.title ?? t('任务'))
              : t('未关联任务 · 专注中')
            : t('准备开始一段专注')}
        </p>
        {elapsed ? (
        ...
        <div className="focus-overlay__actions">
          {!isActive ? (
            <>
              <label className="focus-overlay__todo-pick">
                <span>{t('关联任务')}</span>
                <select
                  value={selectedTodoId ?? ''}
                  onChange={(event) => setSelectedTodoId(event.target.value || undefined)}
                  aria-label={t('关联任务')}
                >
