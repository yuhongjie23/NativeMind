...
const scoreByRules = (candidate: RuleCandidateSource, query: RetrievalQuery): number => {
  ...
  return tagScore + keywordScore + sameSource + sameDay;
};
...
export class RetrievalStrategy {
  ...
  async retrieve(
    ...
    // 模型级 Rerank（深度检索）：优先专用 cross-encoder（精度最高），
    // 失败/慢回退生成式 LLM 打分重排，两者都失败保持启发式顺序（C3）
    if (query.deep) {
    ...
    // 轮询顺序：笔记按各自最佳段分排序（agent 重排后就是 agent 的打分顺序）。
