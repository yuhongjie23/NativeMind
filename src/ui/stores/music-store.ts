...
export const useMusicStore = create<MusicState>((set, get) => ({
  ...
  toggle: async () => {
    ...
    // 恢复播放：
    // 1) 已加载过且有 src → 直接续播（保留进度，不从头发）
    // 2) audio 未创建 / src 被清（stop 过）→ 走 playAt 重建（等价于重新选中）
    const element = getAudio();
    if (element.src && element.paused) {
      silenceOthers('library'); // 恢复也要互斥：停掉背景音乐等其它源
      try {
        await element.play();
        set({ playing: true });
        return;
    ...
    const index = get().tracks.findIndex((track) => track.path === current.path);
    if (index >= 0) await get().playAt(index);
  },

  /** 只暂停不清选中曲目（专注/全屏时用，退出后还能继续听）。
   *  暂停不清 activeSource：这样背景音乐知道「用户选过音乐库」，不会在暂停间隙抢播。
   *  只有 stop（真正放弃当前曲目）才清。 */
  pause: () => {
    audio?.pause();
    set({ playing: false });
