...
mod tests {
    ...
    /// 中文 / 含空格与括号的路径：Windows 下 canonicalize 对 Unicode 路径是
    /// 原生支持的，但历史上这里出过问题（如用户 dataDir 带中文、文件名「いのちの名前 (ピアノ).flac」）。
    /// 钉死行为：中文目录 + 中文文件名 + 空格/括号都能通过校验，且 `..` 逃逸仍被拦。
    #[test]
    fn ensure_within_handles_cjk_paths() {
        let root = std::env::temp_dir().join("nativemind_cjk_测试 目录");
        std::fs::create_dir_all(&root).unwrap();
        let file = root.join("学习笔记（第一版） - 副本.txt");
        std::fs::write(&file, b"x").unwrap();

        // 中文目录 + 中文文件名 + 空格 + 括号：应校验通过
        let safe = ensure_within(&root, &file).expect("中文路径应通过校验");
        assert!(safe.exists());
        assert_eq!(safe.file_name().unwrap().to_string_lossy(), file.file_name().unwrap().to_string_lossy());

        // 中文目录里的逃逸仍被拦（含中文的绝对路径逃出根目录）
        let outside = std::env::temp_dir().join("外部中文.txt");
