...
export const useFocusMusicStore = create<FocusMusicState>((set, get) => ({
  ...
  play: async () => {
    ...
    // 已加载过且停在暂停处（当前文件）：直接续播，保持 currentTime，不从头发。
    // 判断依据：有 src + paused + 未播放。重新设 src 会让浏览器从头播放（这就是
    // 暂停后继续从头播的根因）。
    if (element.src && element.paused && !get().playing) {
      try {
        await element.play();
        set({ playing: true, error: undefined });
        return;
      } catch (error) {
        // 续播也被拦（自动播放策略）：走下面的完整加载重试
