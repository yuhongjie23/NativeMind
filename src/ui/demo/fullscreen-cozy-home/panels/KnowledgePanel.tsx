 ...
 */
import { ArrowRight, BookMarked, Plus, Search } from 'lucide-react';
...
export function KnowledgePanel() {
  ...
  const pickAndImportFile = async () => {
    ...
    try {
      ...
      await importNote({ kind: 'path', path: importPath });
    } finally {
  ...
  const importReadableDoc = async (doc: ReadableDoc) => {
    ...
    try {
      await importNote({ kind: 'path', path: doc.path });
  ...
  const submitImport = async () => {
    ...
    await importNote({ kind: 'text', content: draft, title: title.trim() || undefined });
    setDraft('');
    setTitle('');
  };
  ...
  return (
    <div className="cozy-knowledge">
      ...
      >
        {viewingNote ? (
          <NoteViewer content={viewingNote.content} pageRanges={viewingNote.pageRanges} />
        ) : null}
      </Modal>
    </div>
  );
