...
export class InMemoryCompanionInteractionRepository implements CompanionInteractionRepository {
  ...
  async listRecent(limit = 20): Promise<CompanionInteraction[]> {
...
export class InMemoryAppUsageRepository implements AppUsageRepository {
  private readonly records = new Map<string, AppUsage>();
  ...
  async add(date: string, appActiveSeconds: number, focusSeconds: number): Promise<void> {
    ...
    this.records.set(
      ...
      existing
        ? {
            ...existing,
            appActiveSeconds: existing.appActiveSeconds + appActiveSeconds,
...
export class InMemoryKnowledgeLinkRepository implements KnowledgeLinkRepository {
  ...
  /** 实体被删（如笔记删除）时清理它关联的全部边，两侧都要删 */
  async deleteByEntity(entity: { type: LinkEntityType; id: UUID }): Promise<void> {
    for (const [id, link] of this.links) {
      const touches =
        (link.fromType === entity.type && link.fromId === entity.id) ||
        (link.toType === entity.type && link.toId === entity.id);
      if (touches) this.links.delete(id);
    }
  }
...
export class InMemoryActionProposalRepository implements ActionProposalRepository {
  ...
  async expireOlderThan(hours = 24): Promise<number> {
    ...
    for (const [id, proposal] of this.proposals) {
      if (proposal.status === 'pending' && new Date(proposal.createdAt).getTime() < cutoff) {
        this.proposals.set(id, { ...proposal, status: 'expired', decidedAt: new Date().toISOString() });
        count += 1;
      }
    }
    return count;
...
export class TemplateCompanionQuestionPort implements CompanionQuestionPort {
  async generateQuestion(context: { scene: string }): Promise<CompanionUtterance> {
    if (context.scene === 'focus_complete')
      return { content: 'Nice finish. Want to jot down one sentence about what moved?', emotion: 'happy', quickReplies: ['记一笔', '先不'] };
    if (context.scene === 'repeatedly_aborted')
      return { content: 'Would a smaller next step make this easier to restart?', emotion: 'concerned', quickReplies: ['拆小一点', '先歇'] };
    return { content: 'What is the next small thing worth doing?', emotion: 'curious', quickReplies: ['继续', '先歇会儿'] };
  }
  ...
  async generateFeedback(context: {
    ...
    return {
      content: context.userResponse.trim()
        ? 'Saved. Keep it concrete and light.'
        : 'No worries. We can leave it blank for now.',
      emotion: 'calm',
      quickReplies: context.conversationTurn && context.conversationTurn < 4 ? ['下一步', '就到这里'] : [],
    };
  }
...
export class LocalNoteSearchPort implements NoteSearchPort {
  ...
  async search(query: string, limit: number): Promise<SearchHit[]> {
    ...
    // 段落级匹配：命中返回段落（带 charStart）而不是整条笔记，
    // 和桌面端 chunk 级检索行为一致，用户能定位到具体哪一段命中。
    ...
    for (const note of notes) {
      ...
      // 标签也参与匹配：搜索词恰好是某笔记标签时，即使正文没这个词也该命中
...
export interface LocalDemoRuntime {
  ...
  repositories: {
    ...
    appUsage: InMemoryAppUsageRepository;
...
export const createLocalDemoRuntime = (options: LocalDemoOptions = {}): LocalDemoRuntime => {
  ...
  const appUsage = new InMemoryAppUsageRepository();
